...
plt2 <- ggplot(data_scale,
  ...
  theme_tufte(base_family = 'sans') +
  scale_y_continuous(breaks = seq(0, 125, by = 25)) +
  labs(title = 'Used Toyota Corolla Log Sales Prices',
       x = 'Log Sales Price',
       y = 'Count')
...
plt_res_fit =ggplot(mapping = aes(x = fit4$fitted.values, y = fit4$residuals)) +
    ...
    geom_hline(yintercept = -2*sigma(fit4), size = 0.1) +
    labs(title = 'Residuals vs Fitted Values Plot (Figure 4)',
         x = 'Fitted Values',
         y = 'Residuals')
...
plt_QQ <- ggplot(mapping = aes(x = quants,  y = sort(scale(fit4$residuals)))) +
  ...
  geom_abline(intercept = 0,
              ...
              col = 'red',
              size = .2) +
  labs(title = 'QQ-Norm Plot (Figure 5)',
...
```{r echo = FALSE}
# Should we have a table(s) of all the r^2 values and loo_comparisons for each model
# Maybe explain we this specific model was chosen?
# Maybe this goes after the coefficients are printed?
```
